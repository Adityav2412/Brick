...
export default function WelcomeBackScreen() {
  ...
  const daysSince = user.lastStudyDate
    ? Math.floor((Date.now() - new Date(user.lastStudyDate).getTime()) / (1000 * 60 * 60 * 24))
    : null;
      ...
      <div className="pb-10">
        ...
        {hasLongGap && daysSince !== null ? (
          <p className="text-muted-foreground text-base leading-relaxed mb-2">
            It&apos;s been {daysSince} day{daysSince !== 1 ? "s" : ""} since your last session. No
            worries — every comeback counts.
          </p>
        ) : (
          <p className="text-muted-foreground text-base leading-relaxed mb-2">
