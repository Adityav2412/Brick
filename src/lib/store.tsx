...
import { createAutoBackup, createEmergencyBackup } from "./backup";
...
function reducer(state: AppState, action: Action): AppState {
  switch (action.type) {
    ...
    case "UPDATE_SUBJECT": {
      const subjects = state.subjects.map((s) => (s.id === action.subject.id ? action.subject : s));
      const schedule = state.user
    ...
    case "DELETE_SUBJECT": {
      ...
      const subjects = hasProgress
        ? state.subjects.map((s) => (s.id === action.subjectId ? { ...s, archived: true } : s))
    ...
    case "END_SESSION": {
      ...
      const todayEnergyEntry =
        state.user.todayEnergy && state.user.energyDate === today
          ? [
              {
                date: today,
                energy: state.user.todayEnergy,
                completionPct: Math.min(1, completionPct),
              },
            ]
          : [];
      ...
      const prevHouse = getHouseState(
        ...
        {
          fraction: prevFloor,
          totalMinutes: state.user.houseFloorTotalMinutes ?? newSyll.totalMinutes,
        },
      );
      ...
      const stageAdvanced = !nextHouse.stage.isExpansion && nextHouse.level > prevHouse.level;
...
export function StoreProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    try {
      ...
      if (stored) {
        try {
          const rawParsed = JSON.parse(stored) as AppState;
          createAutoBackup(rawParsed, "App launch / pre-hydration snapshot");
        } catch (e) {
        ...
        if (parsed.user?.onboardingComplete) {
          ...
          const alreadyCreditedJune15 =
            ...
            (parsed.sessions ?? []).some((s) => s.date === JUNE_15 && s.completed);
          if (!alreadyCreditedJune15) {
            const activeSub = (parsed.subjects ?? []).find(
              (s) => !s.archived && s.lectures.some((l) => l.status !== "completed"),
            );
            const targetLec = activeSub?.lectures.find((l) => l.status !== "completed");
            if (activeSub && targetLec) {
              ...
              parsed.user = {
                ...
                houseEffortScore: (parsed.user.houseEffortScore ?? 0) + effortBonus,
                houseProgressFloor: nextFloor,
                houseFloorTotalMinutes:
                  nextFloor > prevFloor
                    ...
                    : (parsed.user.houseFloorTotalMinutes ?? syll.totalMinutes),
                lastStudyDate:
                  !parsed.user.lastStudyDate || parsed.user.lastStudyDate < JUNE_15
                    ? JUNE_15
          ...
          const alreadyAppliedToday = parsed.user.lastRecoveryAppliedDate === today;
...
export function getLectureById(subjects: Subject[], subjectId: string, lectureId: string) {
